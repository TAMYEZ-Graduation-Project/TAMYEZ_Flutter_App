import 'package:flutter/material.dart';

import '../../../../../../core/extensions/text_direction_detector_extension.dart';
import '../../../../../../core/presentation/bases/base_stateless_widget.dart';
import '../../../../../../core/presentation/utils/question_answer_options/question_answer_options_util.dart';
import '../../../../domain/entities/get_saved_quiz_entity.dart';

class SingleChoiceQuestionAnswerWidget extends BaseStatelessWidget {
  final SavedQuizQuestionEntity question;

  const SingleChoiceQuestionAnswerWidget({super.key, required this.question});

  @override
  Widget buildWith(BuildContext context, CommonDependency d) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      spacing: 12,
      children: [
        Text(
          question.text,
          textDirection: question.text.textDirection,
          style: d.typography.title,
        ),
        ListView.separated(
          itemCount: question.options.length,
          physics: const NeverScrollableScrollPhysics(),
          shrinkWrap: true,
          separatorBuilder: (context, index) {
            return const SizedBox(height: 12);
          },
          itemBuilder: (context, index) {
            final option = question.options[index];
            return DecoratedBox(
              decoration: BoxDecoration(
                border: Border.all(
                  color: QuestionAnswerOptionsUtil.getOptionBorderColor(
                    option: option,
                    question: question,
                  ),
                ),
                borderRadius: BorderRadius.circular(8),
              ),
              child: ListTile(
                title: Text(option.text, style: d.typography.body),
                leading: QuestionAnswerOptionsUtil.getOptionRadioIcon(
                  option: option,
                  question: question,
                ),
              ),
            );
          },
        ),
        if (!question.isCorrect && question.explanation.isNotEmpty)
          Text(
            '${d.appLocalizations.explanation}:\n${question.explanation}',
            style: d.typography.body,
          ),
      ],
    );
  }
}
